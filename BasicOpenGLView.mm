...
- (void)drawCubies
{
    const unsigned char * verStr = glGetString(GL_VERSION);
...
- (void) rotateCubeFace: (RubikColor) theColor withCW: (bool) isCW
{
    _moveSeq.push_back(ColMove(theColor, isCW));
    curNbMove++;
...
{ // update info string texture
         ...
         )
	{
		...
		NSString * string = [NSString stringWithFormat:@"Number of moves: %d  Camera at (%0.1f, %0.1f, %0.1f) looking at (%0.1f, %0.1f, %0.1f) with %0.1f aperture", curNbMove, camera.viewPos.x, camera.viewPos.y, camera.viewPos.z, camera.viewDir.x, camera.viewDir.y, camera.viewDir.z, camera.aperture];
		if (camStringTex)
    ...
    if ([characters length]) {
		...
		switch (character) {
            ...
            case 'q':
            {
                ...
                ColMoveSeq moveSeq = handler.convertSeq(handler.genSeq());
                for (ColMoveSeq::iterator it = moveSeq.begin(); it != moveSeq.end(); ++it)
...
//                    printf("Fridrich: %lu  Dummy: %lu \n", solveSeq.size(), solveSeq2.size());
            ...
            case 'w':
            case 'W':
                [self rotateCubeFace:white withCW:!isShift];
                break;
            case 'y':
            case 'Y':
                [self rotateCubeFace:yellow withCW:!isShift];
                break;
            case 'g':
            case 'G':
                [self rotateCubeFace:green withCW:!isShift];
                break;
            case 'b':
            case 'B':
                [self rotateCubeFace:blue withCW:!isShift];
                break;
            case 'r':
            case 'R':
                [self rotateCubeFace:red withCW:!isShift];
                break;
            case 'o':
            case 'O':
                [self rotateCubeFace:orange withCW:!isShift];
                break;
            case ' ':
                _cube = Cube();
				[self setNeedsDisplay: YES];
...
-(id) initWithFrame: (NSRect) frameRect
{
    ...
    //NSOpenGLPixelFormat * pf = [BasicOpenGLView defaultPixelFormat];
